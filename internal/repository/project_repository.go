...
func (r *projectRepository) Create(project *model.Project) error {
	return r.BaseRepository.db.Create(project).Error
}
...
func (r *projectRepository) FindByUserID(userID uint, page, size int) ([]*model.Project, int64, error) {
	...
	if err := r.BaseRepository.db.Model(&model.Project{}).Where("user_id = ?", userID).Count(&total).Error; err != nil {
		return nil, 0, err
	}

	offset := (page - 1) * size
	if err := r.BaseRepository.db.Where("user_id = ?", userID).Offset(offset).Limit(size).Find(&projects).Error; err != nil {
		return nil, 0, err
	}
...
func (r *projectRepository) Update(project *model.Project) error {
	return r.BaseRepository.db.Save(project).Error
}

// Delete 删除项目（软删除）
func (r *projectRepository) Delete(id uint) error {
	return r.BaseRepository.db.Delete(&model.Project{}, id).Error
}
